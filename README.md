...
该阶段会导入 `idle` 和全部命名姿态，生成所有有向转场（当前 12 个姿态，即 132 条）；训练的对象是 minimum-jerk 插值策略，输出验证集最大关节误差。它只使用本地姿态库和 MuJoCo，不调用云端或真实硬件。
...
接入麦克风并完成扬声器回声消除后，可设置 `MICROPHONE_ENABLED=true`。正式程序会在本地约 60 ms 的连续语音能量后立即停止当前音频，录到约 0.65 秒静音后调用多语言转写，再使用同一页面和同一音色回答。每页讲述按自然句边界播放并记录当前句；回答结束时再次确认页面，若页面未变，固定说 `Godt spørgsmål. Nu læser vi videre.`，然后从被打断句的下一句继续。单次中文回答不会改变默认丹麦语和后续故事语言。普通单麦克风必须在系统音频层启用声学回声消除，否则扬声器声音可能被误判为孩子插话。
